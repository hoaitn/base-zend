...
<table class="api">
	<tr>
		<td colspan="2" class="header">DoAuthorization</td>
	</tr>
	<tr>
		<td class="field">Order ID:</td>
		<td><input type="text" name="order_id" value=<?php
		echo $order_id?>>
		(Required)</td>
	</tr>
	<tr>
		...
		<td align=left><input type="text" name="amount"
			value=<?php
			echo $amount?>> <select name=currency>
			<?php
			...
			<option
				<?php
				echo (($currency_cd == $currencies [$i]) ? 'selected' : '')?>><?php
				echo $currencies [$i]?></option>
			<?php
			}
			?>
		</select></td>
