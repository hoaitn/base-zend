...
class Admin_MemberController extends Zend_Controller_Action {
	...
	public function indexAction() {
		$this->view->Title = "Manage Members";
		...
		if ($this->getRequest ()->getParam ( 'keyword' )) {
			switch ($this->getRequest ()->getParam ( 'condition' )) {
				...
				case 'address' :
					...
					break;
				case 'email' :
					$condition ['email LIKE ?'] = "%{$this->getRequest ()->getParam ( 'keyword' )}%";
					break;
			}
		}
		if ($this->getRequest ()->getParam ( 'type' ) > 0) {
			$condition ['member_type =?'] = $this->getRequest ()->getParam ( 'type' );
		}
...
//		}
	...
	public function deleteAction() {
		...
		if ($Member) {
			if ($this->getRequest ()->isPost ()) {
				$Member->delete ();
				$this->Member->log ( 'Member: ' . $Member->username . '(' . $this->getRequest ()->getParam ( 'id' ) . ')', 'Delete' );
				My_Plugin_Libs::setSplash('Delete: <b>'.$Content->content_title.'</b> have been completed.');
				$this->_redirect ( $this->_helper->url ( 'index', 'member', 'admin' ) );
			}
			$this->view->Member = $Member;
		}
